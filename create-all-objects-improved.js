...
async function createAllObjectsImproved() {
  try {
    console.log('🏗️ СОЗДАНИЕ ВСЕХ ОБЪЕКТОВ ИЗ CONTEXT_1.MD (УЛУЧШЕННАЯ ВЕРСИЯ)');
    ...
    // Парсим данные с улучшенной фильтрацией
    ...
    for (let i = 1; i < lines.length; i++) { // Пропускаем заголовок
      ...
      if (techTask && techTask.trim() && !site && !zone && !roomGroup && !room && !cleaningObject) {
