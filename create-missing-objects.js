...
async function createMissingObjects() {
  try {
    console.log('🏗️ СОЗДАНИЕ ВСЕХ НЕДОСТАЮЩИХ ОБЪЕКТОВ');
    ...
    if (!fs.existsSync(realObjectsPath)) {
      console.error('❌ Файл real-objects-list.json не найден!');
      return;
    }
    ...
    for (const objectInfo of realObjectsList) {
      console.log(`\n🏢 Проверяем: ${objectInfo.name}`);

      // Проверяем, существует ли объект
      const existingObject = await prisma.cleaningObject.findFirst({
        where: { name: objectInfo.name }
      });
      ...
      if (!existingObject) {
        ...
        console.log(`   📍 Адрес: ${objectInfo.address}`);
      } else {
        existingCount++;
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🎉 СОЗДАНИЕ БАЗОВЫХ ОБЪЕКТОВ ЗАВЕРШЕНО!');
    console.log(`✅ Создано новых объектов: ${createdCount}`);
    console.log(`📝 Уже существовало: ${existingCount}`);
    console.log(`🏢 Всего объектов: ${realObjectsList.length}`);

    // Теперь запускаем полное создание структуры
    console.log('\n' + '='.repeat(80));
    console.log('🔄 ЗАПУСК СОЗДАНИЯ ПОЛНОЙ СТРУКТУРЫ...\n');
    ...
    console.log(`🏢 Объектов с данными: ${objectsData.size}\n`);

    // Создаем структуру для каждого объекта
    ...
    for (const [objectName, objectData] of objectsData) {
      console.log(`\n🏢 Добавляем структуру для: ${objectName}`);
      ...
      if (!dbObject) {
        console.log(`   ❌ Объект не найден в базе`);
        continue;
      }
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🎉 ПОЛНОЕ СОЗДАНИЕ ВСЕХ ОБЪЕКТОВ ЗАВЕРШЕНО!');
    console.log(`🏢 Всего объектов в системе: ${realObjectsList.length}`);
