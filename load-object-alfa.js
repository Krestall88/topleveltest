...
async function loadObjectAlfa() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ООО «Альфа»');
    console.log('================================\n');

    // Читаем файл и фильтруем строки для Альфа
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const alfaLines = lines.filter(line => line.includes('ООО «Альфа»'));
    console.log(`📋 Найдено строк для ООО «Альфа»: ${alfaLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    alfaLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'ООО «Альфа»';
    const objectAddress = 'г. Самара, ТЦ Мелодия';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ООО «Альфа» - торговый центр Мелодия (1,100 м²)',
          totalArea: 1100,
          creatorId: admin.id
        }
      });
    ...
    alfaLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ООО «Альфа»';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'ТЦ Мелодия';
        ...
        // Определяем зону и группу помещений для торгового центра
        let finalZoneName = zoneName || 'Торговая зона';
        ...
        if (objLower.includes('магазин') || techLower.includes('магазин') ||
            objLower.includes('торговый') || techLower.includes('торговый')) {
          finalZoneName = 'Торговые помещения';
          finalRoomGroupName = 'Магазины';
          ...
          finalRoomGroupName = 'Туалеты';
                   ...
                   objLower.includes('атриум') || techLower.includes('атриум')) {
          ...
          finalRoomGroupName = 'Коридоры и холлы';
                   ...
                   objLower.includes('парковка') || techLower.includes('парковка')) {
          finalZoneName = 'Внешняя территория';
          finalRoomGroupName = 'Придомовая территория';
        } else if (objLower.includes('склад') || techLower.includes('склад')) {
          finalZoneName = 'Складская зона';
          finalRoomGroupName = 'Склады';
        } else if (objLower.includes('кафе') || techLower.includes('кафе') ||
                   objLower.includes('ресторан') || techLower.includes('ресторан') ||
                   objLower.includes('фудкорт') || techLower.includes('фудкорт')) {
          finalZoneName = 'Зона питания';
          finalRoomGroupName = 'Кафе и рестораны';
        }
        ...
        const record = {
          objectName: objectName.replace('1100 м2', '').trim(), // убираем площадь из названия
          siteName: siteName || 'ТЦ Мелодия',
        ...
        alfaRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${alfaRecords.length}\n`);
    ...
    for (const [index, record] of alfaRecords.entries()) {
      try {
        const techCardId = `alfa_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${alfaRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${alfaRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ООО «АЛЬФА»" ЗАВЕРШЕНА!');
