...
async function loadObjectBystrobank() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ПАО «БыстроБанк»');
    console.log('==================================\n');

    // Читаем файл и фильтруем строки для БыстроБанк
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const bystrobankLines = lines.filter(line => line.includes('БыстроБанк'));
    console.log(`📋 Найдено строк для БыстроБанк: ${bystrobankLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    bystrobankLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'ПАО «БыстроБанк»';
    const objectAddress = 'г. Самара, банковское учреждение';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ПАО «БыстроБанк» - банковское учреждение',
          creatorId: admin.id
        }
      });
    ...
    const bystrobankRecords = [];

    bystrobankLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ПАО «БыстроБанк»';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Банковское учреждение';
        ...
        // Определяем зону и группу помещений для банка
        let finalZoneName = zoneName || 'Банковская зона';
        ...
        if (objLower.includes('операционный') || techLower.includes('операционный') ||
            objLower.includes('касса') || techLower.includes('касса')) {
          finalZoneName = 'Операционная зона';
          finalRoomGroupName = 'Операционные залы и кассы';
        } else if (objLower.includes('кабинет') || techLower.includes('кабинет') ||
        ...
        } else if (objLower.includes('сейф') || techLower.includes('сейф') ||
                   objLower.includes('хранилище') || techLower.includes('хранилище')) {
          finalZoneName = 'Зона безопасности';
          finalRoomGroupName = 'Сейфы и хранилища';
        } else if (objLower.includes('банкомат') || techLower.includes('банкомат') ||
                   objLower.includes('терминал') || techLower.includes('терминал')) {
          finalZoneName = 'Зона самообслуживания';
          finalRoomGroupName = 'Банкоматы и терминалы';
        }
        ...
        const record = {
          ...
          siteName: siteName || 'Банковское учреждение',
        ...
        bystrobankRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${bystrobankRecords.length}\n`);
    ...
    const groupedData = {};
    bystrobankRecords.forEach(record => {
    ...
    for (const [index, record] of bystrobankRecords.entries()) {
      try {
        const techCardId = `bystrobank_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${bystrobankRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${bystrobankRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "БЫСТРОБАНК" ЗАВЕРШЕНА!');
