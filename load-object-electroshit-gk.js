...
async function loadObjectElectroshitGK() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ЗАО «ГК «ЭЛЕКТРОЩИТ» ТМ САМАРА»');
    console.log('==================================================\n');

    // Читаем файл и фильтруем строки для ГК Электрощит
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const electroshitLines = lines.filter(line => line.includes('ГК «ЭЛЕКТРОЩИТ» ТМ САМАРА'));
    console.log(`📋 Найдено строк для ГК Электрощит: ${electroshitLines.length}\n`);
    ...
    const objectName = 'ЗАО «ГК «ЭЛЕКТРОЩИТ» ТМ САМАРА»';
    const objectAddress = 'г. Самара, промышленная зона';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'Группа компаний ЭЛЕКТРОЩИТ - крупный промышленный комплекс (159 968.55 м²)',
          totalArea: 159968.55,
          creatorId: admin.id
        }
      });
    ...
    console.log(`   Площадь: 159,968.55 м²\n`);
    ...
    electroshitLines.forEach((line, index) => {
      ...
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ЗАО «ГК «ЭЛЕКТРОЩИТ» ТМ САМАРА»';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Промышленный комплекс';
        ...
        // Определяем зону и группу помещений
        ...
        if (freqLower.includes('ежедневно') || freqLower.includes('1 раз в день') || freqLower.includes('1раз в день')) {
        ...
        } else if (freqLower.includes('по мере загрязнения') || freqLower.includes('по мере необходимости')) {
        ...
        const record = {
          objectName: objectName.replace('159 968.55 м2', '').trim(), // убираем площадь из названия
          siteName: siteName || 'Промышленный комплекс',
    ...
    for (const [index, record] of electroshitRecords.entries()) {
      try {
        const techCardId = `electroshit_gk_${index + 1}_${Date.now()}`;
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ГК ЭЛЕКТРОЩИТ" ЗАВЕРШЕНА!');
