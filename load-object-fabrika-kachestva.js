...
async function loadObjectFabrikaKachestva() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ООО «Производственная компания ФАБРИКА КАЧЕСТВА»');
    console.log('================================================================\n');

    // Читаем файл и фильтруем строки для ФАБРИКА КАЧЕСТВА
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const fabrikaLines = lines.filter(line => line.includes('ФАБРИКА КАЧЕСТВА'));
    console.log(`📋 Найдено строк для ФАБРИКА КАЧЕСТВА: ${fabrikaLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    fabrikaLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'ООО «Производственная компания ФАБРИКА КАЧЕСТВА»';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ООО «Производственная компания ФАБРИКА КАЧЕСТВА» - производственное предприятие',
          creatorId: admin.id
        }
      });
    ...
    const fabrikaRecords = [];

    fabrikaLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ООО «Производственная компания ФАБРИКА КАЧЕСТВА»';
        ...
        fabrikaRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${fabrikaRecords.length}\n`);
    ...
    for (const [index, record] of fabrikaRecords.entries()) {
      try {
        const techCardId = `fabrika_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${fabrikaRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${fabrikaRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ФАБРИКА КАЧЕСТВА" ЗАВЕРШЕНА!');
