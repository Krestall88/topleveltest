...
async function loadObjectKompaktiv() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: КОМПАКТИВ');
    console.log('=============================\n');

    // Читаем файл и фильтруем строки для КОМПАКТИВ
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const kompaktivLines = lines.filter(line => 
      line.includes('КОМПАКТИВ')
    );
    console.log(`📋 Найдено строк для КОМПАКТИВ: ${kompaktivLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    kompaktivLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'КОМПАКТИВ';
    const objectAddress = 'г. Самара, офисное здание';

    let cleaningObject = await prisma.cleaningObject.findFirst({
      where: { 
        OR: [
          { name: objectName },
          { name: { contains: 'КОМПАКТИВ' } }
        ]
      }
    });
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'КОМПАКТИВ - офисное здание',
          creatorId: admin.id
        }
      });
    ...
    const kompaktivRecords = [];

    kompaktivLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectNameFromFile = parts[0]?.trim() || '';
        
        // Проверяем, что это действительно строка с КОМПАКТИВ
        if (!objectNameFromFile.includes('КОМПАКТИВ')) {
          return;
        }
        ...
        // Определяем зону и группу помещений для офисного здания
        ...
        // Анализируем тип помещения по названию для офисного здания
        ...
        if (objLower.includes('офис') || techLower.includes('офис') ||
          ...
          finalRoomGroupName = 'Серверные и технические';
          ...
          finalRoomGroupName = 'Склады и архивы';
        ...
        kompaktivRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${kompaktivRecords.length}\n`);

    if (kompaktivRecords.length === 0) {
      console.log('⚠️ Не найдено данных для КОМПАКТИВ');
    ...
    const groupedData = {};
    kompaktivRecords.forEach(record => {
    ...
    for (const [index, record] of kompaktivRecords.entries()) {
      try {
        const techCardId = `kompaktiv_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${kompaktivRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${kompaktivRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "КОМПАКТИВ" ЗАВЕРШЕНА!');
