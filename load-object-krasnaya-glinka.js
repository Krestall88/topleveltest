...
async function loadObjectKrasnayaGlinka() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: УФПСО санаторий «Красная Глинка»');
    console.log('===============================================\n');

    // Читаем файл и фильтруем строки для Красная Глинка
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const krasnayaGlinkaLines = lines.filter(line => line.includes('Красная Глинка'));
    console.log(`📋 Найдено строк для Красная Глинка: ${krasnayaGlinkaLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    krasnayaGlinkaLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'УФПСО санаторий «Красная Глинка»';
    const objectAddress = 'г. Самара, санаторий Красная Глинка';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'УФПСО санаторий «Красная Глинка» - санаторно-курортное учреждение',
          creatorId: admin.id
        }
      });
    ...
    const krasnayaGlinkaRecords = [];

    krasnayaGlinkaLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'УФПСО санаторий «Красная Глинка»';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Санаторий';
        ...
        // Определяем зону и группу помещений для санатория
        let finalZoneName = zoneName || 'Лечебная зона';
        ...
        // Анализируем тип помещения по названию для санатория
        const techLower = cleanTechCardName.toLowerCase();
        const objLower = cleaningObjectName.toLowerCase();
        
        if (objLower.includes('палата') || techLower.includes('палата') ||
            objLower.includes('номер') || techLower.includes('номер')) {
          finalZoneName = 'Жилая зона';
          finalRoomGroupName = 'Палаты и номера';
        } else if (objLower.includes('процедурная') || techLower.includes('процедурная') ||
                   objLower.includes('лечебная') || techLower.includes('лечебная') ||
                   objLower.includes('медицинская') || techLower.includes('медицинская')) {
          finalZoneName = 'Лечебная зона';
          finalRoomGroupName = 'Процедурные кабинеты';
        } else if (objLower.includes('столовая') || techLower.includes('столовая') ||
                   objLower.includes('ресторан') || techLower.includes('ресторан') ||
                   objLower.includes('кафе') || techLower.includes('кафе')) {
          finalZoneName = 'Зона питания';
          finalRoomGroupName = 'Столовая и кафе';
        } else if (objLower.includes('холл') || techLower.includes('холл') ||
                   objLower.includes('фойе') || techLower.includes('фойе') ||
                   objLower.includes('вестибюль') || techLower.includes('вестибюль')) {
          finalZoneName = 'Общие зоны';
          finalRoomGroupName = 'Холлы и фойе';
        } else if (objLower.includes('санузел') || techLower.includes('санузел') ||
                   objLower.includes('туалет') || techLower.includes('туалет') ||
                   objLower.includes('ванная') || techLower.includes('ванная')) {
          finalZoneName = 'Санитарные помещения';
          finalRoomGroupName = 'Санузлы';
        } else if (objLower.includes('коридор') || techLower.includes('коридор') ||
                   objLower.includes('переход') || techLower.includes('переход')) {
          finalZoneName = 'Общие зоны';
          finalRoomGroupName = 'Коридоры';
        } else if (objLower.includes('лестниц') || techLower.includes('лестниц')) {
          finalZoneName = 'Лестничные клетки';
          finalRoomGroupName = 'Лестницы';
        } else if (objLower.includes('спортзал') || techLower.includes('спортзал') ||
                   objLower.includes('тренажерный') || techLower.includes('тренажерный') ||
                   objLower.includes('бассейн') || techLower.includes('бассейн')) {
          finalZoneName = 'Спортивная зона';
          finalRoomGroupName = 'Спортзалы и бассейн';
        } else if (objLower.includes('библиотека') || techLower.includes('библиотека') ||
                   objLower.includes('читальный') || techLower.includes('читальный')) {
          finalZoneName = 'Досуговая зона';
          finalRoomGroupName = 'Библиотека';
        } else if (objLower.includes('администрация') || techLower.includes('администрация') ||
        ...
        } else if (objLower.includes('прачечная') || techLower.includes('прачечная') ||
                   objLower.includes('хозяйственная') || techLower.includes('хозяйственная')) {
          finalZoneName = 'Хозяйственная зона';
          finalRoomGroupName = 'Прачечная и хозблок';
        } else if (objLower.includes('территория') || techLower.includes('территория') ||
                   objLower.includes('парк') || techLower.includes('парк')) {
          finalZoneName = 'Территория санатория';
          finalRoomGroupName = 'Парковая зона';
        }
        ...
        } else if (techLower.includes('смена белья') || techLower.includes('постельное')) {
          workType = 'Смена белья';
        } else if (techLower.includes('уход за растениями') || techLower.includes('полив')) {
          workType = 'Уход за растениями';
        }
        ...
        krasnayaGlinkaRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${krasnayaGlinkaRecords.length}\n`);
    ...
    const groupedData = {};
    krasnayaGlinkaRecords.forEach(record => {
    ...
    for (const [index, record] of krasnayaGlinkaRecords.entries()) {
      try {
        const techCardId = `krasnayaglinka_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${krasnayaGlinkaRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${krasnayaGlinkaRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "КРАСНАЯ ГЛИНКА" ЗАВЕРШЕНА!');
