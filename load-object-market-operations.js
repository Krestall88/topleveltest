...
async function loadObjectMarketOperations() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ООО «Маркет.Операции»');
    console.log('=========================================\n');

    // Читаем файл и фильтруем строки для Маркет.Операции
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const marketLines = lines.filter(line => line.includes('Маркет.Операции'));
    console.log(`📋 Найдено строк для Маркет.Операции: ${marketLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    marketLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectAddress = 'г. Самара, логистический центр';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'Логистический центр Маркет.Операции - крупный складской комплекс (26,188.60 м²)',
          totalArea: 26188.60,
          creatorId: admin.id
        }
      });
    ...
    console.log(`   Площадь: 26,188.60 м²\n`);
    ...
    marketLines.forEach((line, index) => {
      ...
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ООО «Маркет.Операции»';
        ...
        if (objLower.includes('склад') || techLower.includes('склад')) {
          ...
          finalZoneName = 'Логистическая зона';
          finalRoomGroupName = 'Зона погрузки-разгрузки';
        }
        ...
        const record = {
          objectName: objectName.replace('26188,60  м2', '').trim(), // убираем площадь из названия
    ...
    console.log(`📋 Обработано записей: ${marketRecords.length}\n`);
    ...
    for (const [index, record] of marketRecords.entries()) {
      try {
        const techCardId = `market_ops_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${marketRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${marketRecords.length}`);
