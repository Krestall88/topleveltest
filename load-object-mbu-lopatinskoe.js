...
async function loadObjectMBULopatinskoe() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: МБУ «Лопатинское»');
    console.log('===================================\n');

    // Читаем файл и фильтруем строки для МБУ «Лопатинское»
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const lopatinskoLines = lines.filter(line => 
      line.includes('МБУ «Лопатинское»') || line.includes('Лопатинское')
    );
    console.log(`📋 Найдено строк для МБУ «Лопатинское»: ${lopatinskoLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    lopatinskoLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'МБУ «Лопатинское»';
    const objectAddress = 'г. Самара, муниципальное бюджетное учреждение';

    let cleaningObject = await prisma.cleaningObject.findFirst({
      where: { 
        OR: [
          { name: objectName },
          { name: { contains: 'Лопатинское' } }
        ]
      }
    });
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'МБУ «Лопатинское» - муниципальное бюджетное учреждение',
          creatorId: admin.id
        }
      });
    ...
    const lopatinskoRecords = [];

    lopatinskoLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectNameFromFile = parts[0]?.trim() || '';
        
        // Проверяем, что это действительно строка с МБУ «Лопатинское»
        if (!objectNameFromFile.includes('Лопатинское')) {
          return;
        }

        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Муниципальное учреждение';
        ...
        // Определяем зону и группу помещений для муниципального учреждения
        let finalZoneName = zoneName || 'Административная зона';
        ...
        // Анализируем тип помещения по названию для муниципального учреждения
        ...
        if (objLower.includes('офис') || techLower.includes('офис') ||
        ...
        } else if (objLower.includes('зал') || techLower.includes('зал') ||
                   objLower.includes('конференц') || techLower.includes('конференц') ||
                   objLower.includes('актовый') || techLower.includes('актовый')) {
          finalZoneName = 'Общественная зона';
          finalRoomGroupName = 'Залы и аудитории';
          ...
          finalRoomGroupName = 'Столовая и буфеты';
                   ...
                   objLower.includes('архив') || techLower.includes('архив')) {
          finalZoneName = 'Информационная зона';
          finalRoomGroupName = 'Библиотека и архивы';
        } else if (objLower.includes('медицинский') || techLower.includes('медицинский') ||
                   objLower.includes('медпункт') || techLower.includes('медпункт')) {
          ...
          finalZoneName = 'Территория учреждения';
        ...
        const record = {
          ...
          siteName: siteName || 'Муниципальное учреждение',
        ...
        lopatinskoRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${lopatinskoRecords.length}\n`);

    if (lopatinskoRecords.length === 0) {
      console.log('⚠️ Не найдено данных для МБУ «Лопатинское»');
    ...
    const groupedData = {};
    lopatinskoRecords.forEach(record => {
    ...
    for (const [index, record] of lopatinskoRecords.entries()) {
      try {
        const techCardId = `lopatinskoe_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${lopatinskoRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${lopatinskoRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "МБУ ЛОПАТИНСКОЕ" ЗАВЕРШЕНА!');
