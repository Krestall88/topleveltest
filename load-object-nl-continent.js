...
async function loadObjectNLContinent() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ООО «НЛ Континент»');
    console.log('===================================\n');

    // Читаем файл и фильтруем строки для НЛ Континент
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const nlContinentLines = lines.filter(line => line.includes('НЛ Континент'));
    console.log(`📋 Найдено строк для НЛ Континент: ${nlContinentLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    nlContinentLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'ООО «НЛ Континент»';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ООО «НЛ Континент» - офисное здание (144.2 м²)',
          totalArea: 144.2,
          creatorId: admin.id
        }
      });
    ...
    const nlContinentRecords = [];

    nlContinentLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ООО «НЛ Континент»';
        ...
        // Определяем зону и группу помещений для небольшого офиса
        ...
        const record = {
          objectName: objectName.replace('144,2 м2', '').trim(), // убираем площадь из названия
        ...
        nlContinentRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${nlContinentRecords.length}\n`);
    ...
    const groupedData = {};
    nlContinentRecords.forEach(record => {
    ...
    for (const [index, record] of nlContinentRecords.entries()) {
      try {
        const techCardId = `nlcontinent_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${nlContinentRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${nlContinentRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "НЛ КОНТИНЕНТ" ЗАВЕРШЕНА!');
