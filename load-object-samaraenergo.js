...
async function loadObjectSamaraenergo() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: Самараэнерго');
    console.log('===============================\n');
    ...
    const samaraenergoLines = lines.filter(line => line.includes('Самараэнерго'));
    ...
    samaraenergoLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'Самараэнерго';
    const objectAddress = 'г. Самара, энергетическая компания';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'Самараэнерго - энергетическая компания (429.2 м²)',
          totalArea: 429.2,
          creatorId: admin.id
        }
      });
    ...
    const samaraenergoRecords = [];

    samaraenergoLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'Самараэнерго';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Энергетическая компания';
        ...
        // Определяем зону и группу помещений для энергетической компании
        ...
        } else if (objLower.includes('диспетчерская') || techLower.includes('диспетчерская') ||
                   objLower.includes('пульт') || techLower.includes('пульт')) {
          finalZoneName = 'Диспетчерская зона';
          finalRoomGroupName = 'Диспетчерские';
        } else if (objLower.includes('серверная') || techLower.includes('серверная') ||
                   objLower.includes('техническая') || techLower.includes('техническая')) {
          finalZoneName = 'Технические помещения';
          finalRoomGroupName = 'Серверные и техпомещения';
        ...
        const record = {
          objectName: objectName.replace('429,2 м2', '').trim(), // убираем площадь из названия
          siteName: siteName || 'Энергетическая компания',
        ...
        samaraenergoRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${samaraenergoRecords.length}\n`);
    ...
    const groupedData = {};
    samaraenergoRecords.forEach(record => {
    ...
    for (const [index, record] of samaraenergoRecords.entries()) {
      try {
        const techCardId = `samaraenergo_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${samaraenergoRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${samaraenergoRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "САМАРАЭНЕРГО" ЗАВЕРШЕНА!');
