...
async function loadObjectSamGMU() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ФГБОУ ВО СамГМУ Минздрава России');
    console.log('================================================\n');

    // Читаем файл и фильтруем строки для СамГМУ
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const samgmuLines = lines.filter(line => line.includes('СамГМУ'));
    console.log(`📋 Найдено строк для СамГМУ: ${samgmuLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    samgmuLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'ФГБОУ ВО СамГМУ Минздрава России';
    const objectAddress = 'г. Самара, медицинский университет';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ФГБОУ ВО СамГМУ Минздрава России - медицинский университет',
          creatorId: admin.id
        }
      });
    ...
    samgmuLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'ФГБОУ ВО СамГМУ Минздрава России';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Медицинский университет';
        ...
        // Определяем зону и группу помещений для медицинского университета
        let finalZoneName = zoneName || 'Учебная зона';
        ...
        if (objLower.includes('аудитория') || techLower.includes('аудитория') ||
            objLower.includes('лекционный') || techLower.includes('лекционный')) {
          finalZoneName = 'Учебные аудитории';
          finalRoomGroupName = 'Лекционные залы';
        } else if (objLower.includes('лаборатория') || techLower.includes('лаборатория') ||
                   objLower.includes('практикум') || techLower.includes('практикум')) {
          finalZoneName = 'Лабораторная зона';
          finalRoomGroupName = 'Лаборатории';
        } else if (objLower.includes('кафедра') || techLower.includes('кафедра') ||
                   objLower.includes('деканат') || techLower.includes('деканат')) {
          finalZoneName = 'Административная зона';
          finalRoomGroupName = 'Кафедры и деканаты';
          ...
          finalZoneName = 'Библиотечная зона';
        ...
        } else if (objLower.includes('актовый') || techLower.includes('актовый') ||
                   objLower.includes('конференц') || techLower.includes('конференц')) {
          finalZoneName = 'Актовые залы';
          finalRoomGroupName = 'Конференц-залы';
        } else if (objLower.includes('спортзал') || techLower.includes('спортзал') ||
                   objLower.includes('спортивный') || techLower.includes('спортивный')) {
          finalZoneName = 'Спортивная зона';
          finalRoomGroupName = 'Спортзалы';
        } else if (objLower.includes('медицинский') || techLower.includes('медицинский') ||
                   objLower.includes('клиника') || techLower.includes('клиника')) {
          finalZoneName = 'Медицинская зона';
          finalRoomGroupName = 'Медицинские кабинеты';
        ...
        const record = {
          ...
          siteName: siteName || 'Медицинский университет',
        ...
        samgmuRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${samgmuRecords.length}\n`);
    ...
    for (const [index, record] of samgmuRecords.entries()) {
      try {
        const techCardId = `samgmu_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${samgmuRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${samgmuRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "САМГМУ" ЗАВЕРШЕНА!');
