...
async function loadObjectTyazhmash() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: Акционерное общество "ТЯЖМАШ"');
    console.log('===============================================\n');

    // Читаем файл и фильтруем строки для ТЯЖМАШ
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const tyazhLines = lines.filter(line => line.includes('ТЯЖМАШ'));
    console.log(`📋 Найдено строк для ТЯЖМАШ: ${tyazhLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    tyazhLines.slice(0, 3).forEach((line, index) => {
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'Акционерное общество ТЯЖМАШ - машиностроительный завод',
          creatorId: admin.id
        }
      });
    ...
    tyazhLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'АО "ТЯЖМАШ"';
        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Машиностроительный завод';
        ...
        // Определяем зону и группу помещений для машиностроительного завода
        ...
        const record = {
          objectName: objectName.replace('АО "ТЯЖМАШ"', 'АО "ТЯЖМАШ"').trim(),
          siteName: siteName || 'Машиностроительный завод',
        ...
        tyazhRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${tyazhRecords.length}\n`);
    ...
    for (const [index, record] of tyazhRecords.entries()) {
      try {
        const techCardId = `tyazhmash_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${tyazhRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${tyazhRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ТЯЖМАШ" ЗАВЕРШЕНА!');
