...
async function loadObjectVolgar() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: Волгарь (ЖилЭнерго, ЖЭУ-66)');
    console.log('===============================================\n');

    // Читаем файл и фильтруем строки для Волгаря
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    const volgarLines = lines.filter(line => line.includes('Волгарь') && line.includes('ЖилЭнерго'));
    console.log(`📋 Найдено строк для Волгаря: ${volgarLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    volgarLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = 'Волгарь (ЖилЭнерго, ЖЭУ-66)';
    const objectAddress = 'Жилой комплекс Волгарь';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'Жилой комплекс - уборка мест общего пользования и придомовой территории',
          creatorId: admin.id
        }
      });
    ...
    volgarLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectName = parts[0]?.trim() || 'Волгарь (ЖилЭнерго, ЖЭУ-66)';
        ...
        } else if (techLower.includes('уборка мусора') || techLower.includes('вынос мусора')) {
          workType = 'Уборка мусора';
        } else if (techLower.includes('газон') || techLower.includes('полив')) {
        ...
        } else if (freqLower.includes('теплый период')) {
          cleanPeriod = 'Летний период';
        } else if (freqLower.includes('холодный период')) {
        ...
        volgarRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${volgarRecords.length}\n`);
    ...
    for (const [index, record] of volgarRecords.entries()) {
      try {
        const techCardId = `volgar_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 5 === 0) {
          console.log(`✅ Загружено ${index + 1}/${volgarRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${volgarRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ВОЛГАРЬ" ЗАВЕРШЕНА!');
