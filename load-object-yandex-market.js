...
async function loadObjectYandexMarket() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: ООО «Яндекс.Маркет»');
    console.log('===================================\n');

    // Читаем файл и фильтруем строки для Маркет.Операции (склад Яндекс)
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    // Ищем данные для Маркет.Операции (это склад Яндекс.Маркет)
    const yandexLines = lines.filter(line => 
      line.includes('Маркет.Операции')
    );
    console.log(`📋 Найдено строк для Маркет.Операции: ${yandexLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    yandexLines.slice(0, 5).forEach((line, index) => {
    ...
    const objectName = 'ООО «Маркет.Операции»';
    const objectAddress = 'Самарская область, с. Преображенка, ул. Индустриальная, здание № 2А/5';

    let cleaningObject = await prisma.cleaningObject.findFirst({
      where: { 
        OR: [
          { name: objectName },
          { name: { contains: 'Маркет.Операции' } }
        ]
      }
    });
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: 'ООО «Маркет.Операции» - логистический центр Яндекс.Маркет (26,188.60 м²)',
          creatorId: admin.id
        }
      });
    ...
    yandexLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectNameFromFile = parts[0]?.trim() || '';
        
        // Проверяем, что это действительно строка с Маркет.Операции
        if (!objectNameFromFile.includes('Маркет.Операции')) {
          return;
        }

        const address = parts[1]?.trim() || '';
        const siteName = parts[2]?.trim() || 'Логистический центр';
        ...
        // Определяем зону и группу помещений для логистического центра
        let finalZoneName = zoneName || 'Складская зона';
        ...
        // Анализируем тип помещения по названию для логистического центра
        const techLower = cleanTechCardName.toLowerCase();
        const objLower = cleaningObjectName.toLowerCase();
        
        if (objLower.includes('склад') || techLower.includes('склад') ||
            objLower.includes('хранение') || techLower.includes('хранение')) {
          finalZoneName = 'Складская зона';
          finalRoomGroupName = 'Склады и хранилища';
        } else if (objLower.includes('погрузка') || techLower.includes('погрузка') ||
                   objLower.includes('разгрузка') || techLower.includes('разгрузка') ||
                   objLower.includes('рампа') || techLower.includes('рампа')) {
          finalZoneName = 'Зона погрузки';
          finalRoomGroupName = 'Погрузочно-разгрузочная зона';
        } else if (objLower.includes('сортировка') || techLower.includes('сортировка') ||
                   objLower.includes('комплектация') || techLower.includes('комплектация')) {
          finalZoneName = 'Зона сортировки';
          finalRoomGroupName = 'Сортировка и комплектация';
          ...
          finalRoomGroupName = 'Туалеты';
                   ...
                   objLower.includes('проход') || techLower.includes('проход')) {
          finalZoneName = 'Общие зоны';
          finalRoomGroupName = 'Коридоры и проходы';
        } else if (objLower.includes('раздевалка') || techLower.includes('раздевалка') ||
                   objLower.includes('бытовая') || techLower.includes('бытовая')) {
          ...
          finalZoneName = 'Территория склада';
        ...
        } else if (techLower.includes('сбор') && (techLower.includes('стретч') || techLower.includes('пленк'))) {
          workType = 'Сбор упаковочных материалов';
        } else if (techLower.includes('сбор') && techLower.includes('гофрокартон')) {
          workType = 'Сбор и утилизация картона';
        } else if (techLower.includes('сбор') && techLower.includes('паллет')) {
          workType = 'Управление паллетами';
        } else if (techLower.includes('поломоечная машина') || techLower.includes('механическая')) {
          workType = 'Механизированная уборка';
        }
        ...
        } else if (freqLower.includes('раз в смену') || freqLower.includes('смена')) {
          if (freqLower.includes('1 раз в смену')) {
            cleanPeriod = 'Раз в смену';
          } else if (freqLower.includes('2 раза в смену')) {
            cleanPeriod = 'Два раза в смену';
          } else if (freqLower.includes('3 раз в смену')) {
            cleanPeriod = 'Три раза в смену';
          } else if (freqLower.includes('6 раз в смену')) {
            cleanPeriod = 'Шесть раз в смену';
          } else {
            cleanPeriod = 'По сменам';
          }
        }

        const record = {
          objectName: objectName, // используем унифицированное название
          siteName: siteName || 'Логистический центр',
        ...
        yandexRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${yandexRecords.length}\n`);
    ...
    for (const [index, record] of yandexRecords.entries()) {
      try {
        const techCardId = `market_operations_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${yandexRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${yandexRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "МАРКЕТ.ОПЕРАЦИИ" ЗАВЕРШЕНА!');
