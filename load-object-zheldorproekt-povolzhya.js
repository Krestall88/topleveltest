...
async function loadObjectZheldorproektPovolzhya() {
  try {
    console.log('🏢 ЗАГРУЗКА ОБЪЕКТА: «Желдорпроект Поволжья»');
    console.log('===========================================\n');

    // Читаем файл и фильтруем строки для «Желдорпроект Поволжья»
    const fileContent = fs.readFileSync('context_1.md', 'utf-8');
    const lines = fileContent.split('\n');
    
    // Ищем строки, которые содержат полное название (может быть разбито на строки)
    const zheldorLines = [];
    for (let i = 0; i < lines.length - 1; i++) {
      const currentLine = lines[i];
      const nextLine = lines[i + 1];
      
      // Проверяем, если текущая строка содержит начало названия, а следующая - продолжение
      if (currentLine.includes('«Желдорпроект Поволжья»') && 
          nextLine && nextLine.includes('филиала АО «Росжелдорпроект»')) {
        // Объединяем строки
        const combinedLine = currentLine.replace('"«Желдорпроект Поволжья» - ', '') + nextLine;
        zheldorLines.push(combinedLine);
      }
    }
    console.log(`📋 Найдено строк для «Желдорпроект Поволжья»: ${zheldorLines.length}\n`);

    // Анализируем первые строки для понимания структуры
    console.log('🔍 АНАЛИЗ СТРУКТУРЫ ДАННЫХ:');
    zheldorLines.slice(0, 3).forEach((line, index) => {
    ...
    const objectName = '«Желдорпроект Поволжья»';
    const objectAddress = 'ул. Красноармейская, 137б';
    ...
    if (!cleaningObject) {
      ...
      cleaningObject = await prisma.cleaningObject.create({
        data: {
          ...
          description: '«Желдорпроект Поволжья» - проектный институт',
          creatorId: admin.id
        }
      });
    ...
    const zheldorRecords = [];

    zheldorLines.forEach((line, index) => {
      const parts = line.split('\t');
      
      if (parts.length >= 8) {
        const objectNameFromFile = parts[0]?.trim() || '';
        
        // Проверяем, что это действительно строка с филиала АО «Росжелдорпроект»
        if (!objectNameFromFile.includes('филиала АО «Росжелдорпроект»')) {
          return;
        }
        ...
        // Анализируем тип помещения по названию для проектного института
        const techLower = cleanTechCardName.toLowerCase();
        const objLower = cleaningObjectName.toLowerCase();
        
        if (objLower.includes('офисные') || objLower.includes('офис') ||
            objLower.includes('кабинет') || techLower.includes('кабинет')) {
          finalZoneName = 'Офисная зона';
          finalRoomGroupName = 'Офисы и кабинеты';
        } else if (objLower.includes('производственные') || objLower.includes('производство') ||
                   objLower.includes('мастерская') || objLower.includes('цех')) {
          finalZoneName = 'Производственная зона';
          finalRoomGroupName = 'Производственные помещения';
        } else if (objLower.includes('санузл') || objLower.includes('туалет') ||
                   techLower.includes('санузл') || techLower.includes('туалет')) {
        ...
        } else if (techLower.includes('удаление мусора') || techLower.includes('вынос мусора')) {
          workType = 'Уборка мусора';
        } else if (techLower.includes('окна')) {
          workType = 'Мойка окон';
        } else if (techLower.includes('полировка') || techLower.includes('чистка мебели')) {
          workType = 'Уход за мебелью';
        } else if (techLower.includes('антибактериальная') || techLower.includes('кулер')) {
          workType = 'Санитарная обработка';
        } else if (techLower.includes('удаление пятен') || techLower.includes('липких материалов')) {
          workType = 'Удаление пятен';
        } else if (techLower.includes('очистка сливных трапов')) {
          workType = 'Техническая очистка';
        }
        ...
        if (freqLower.includes('1 раз в день') || freqLower.includes('– 1 раз в день')) {
          cleanPeriod = 'Ежедневно';
        } else if (freqLower.includes('2 раза в день') || freqLower.includes('– 2 раза в день')) {
          cleanPeriod = 'Два раза в день';
        } else if (freqLower.includes('1 раз в неделю') || freqLower.includes('– 1 раз в неделю')) {
        ...
        } else if (freqLower.includes('5 дней в неделю')) {
        ...
        zheldorRecords.push(record);
      }
    });

    console.log(`📋 Обработано записей: ${zheldorRecords.length}\n`);

    if (zheldorRecords.length === 0) {
      console.log('⚠️ Не найдено данных для «Желдорпроект Поволжья»');
    ...
    for (const [index, record] of zheldorRecords.entries()) {
      try {
        const techCardId = `zheldor_${index + 1}_${Date.now()}`;
        ...
        if ((index + 1) % 10 === 0) {
          console.log(`✅ Загружено ${index + 1}/${zheldorRecords.length} записей`);
        }
    ...
    console.log(`📊 Всего записей: ${zheldorRecords.length}`);
    ...
    console.log('\n✅ ЗАГРУЗКА ОБЪЕКТА "ЖЕЛДОРПРОЕКТ ПОВОЛЖЬЯ" ЗАВЕРШЕНА!');
...
loadObjectZheldorproektPovolzhya();
