...
async function deleteDuplicateYugService() {
  console.log('🗑️ УДАЛЕНИЕ ДУБЛИРУЮЩЕГО ОБЪЕКТА ЮГ-СЕРВИСА (ФИНАЛЬНАЯ ВЕРСИЯ)\n');
  ...
  try {
    ...
    // Сначала проверим, что объект существует (упрощенный запрос)
    ...
    if (sitesWithManagers.length > 0) {
      ...
      sitesWithManagers.forEach(site => {
        console.log(`   - ${site.name}: менеджер ID ${site.managerId}`);
      });
      return;
    }
    ...
    // 3. Удаляем зоны из участков (если есть)
    let totalDeletedSiteZones = 0;
    if (objectToDelete._count.sites > 0) {
      // Находим все зоны, связанные с участками этого объекта
      const sitesWithZones = await prisma.site.findMany({
        where: { objectId: objectToDeleteId },
        include: { zones: true }
      });
      
      for (const site of sitesWithZones) {
    ...
    // 4. Удаляем помещения (без roomGroups, так как их нет в схеме)
