...
async function deleteDuplicateYugService() {
  console.log('🗑️ УДАЛЕНИЕ ДУБЛИРУЮЩЕГО ОБЪЕКТА ЮГ-СЕРВИСА\n');
  ...
  try {
    ...
    // 3. Удаляем помещения (и их зоны)
    if (objectToDelete._count.rooms > 0) {
      // Сначала удаляем зоны в помещениях
      const roomIds = objectToDelete.rooms.map(room => room.id);
    ...
    // 4. Удаляем участки (и их зоны)
    if (objectToDelete._count.sites > 0) {
      // Сначала удаляем зоны в участках
      const siteIds = objectToDelete.sites.map(site => site.id);
      const deletedSiteZones = await prisma.zone.deleteMany({
        where: { siteId: { in: siteIds } }
      });
      console.log(`✅ Удалено зон в участках: ${deletedSiteZones.count}`);
